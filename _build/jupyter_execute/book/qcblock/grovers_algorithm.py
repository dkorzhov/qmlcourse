...
# 
# Этот минус относится не только к вспомогательному кубиту, но и ко всему состоянию, соответствующему индексу $11$, так что можно считать, что вспомогательный кубит остался неизменным, и отнести минус к состоянию $11$ верхних кубитов. Таким образом, индекс $11$ помечен минусом как искомый.
# 
...
# 
# Вектор $c_{1a}$ - это отражение вектора $c$ относительно горизонтальной оси. Договоримся, что буквы $a$ и $b$, которые будут употребляться в нижнем индексе угла $c$ и операторов $U$, обозначают соответственно первую и вторую часть итерации (не путать с векторами $a$ и $b$). Итак, вектор состояния отражается на угол $\theta$ вниз от горизонтальной оси:
# 
...
# ```
# 
# Операция для получения вектора $c_{1b}$ будет выглядеть так:
# 
...
# 
# Мы разобрали случай, когда требуется найти один объект в таблице. Если же потребуется найти несколько объектов, то тогда, обозначив их количество за $K$, мы должны будем проделать около $\frac {\pi}{4} \sqrt {\frac{N}{K}}$ итераций.
# 
# 
...
# ## Задание
# 
# 1. Распишите операторы $U_{1a}$ и $U_{1b}$ из примера в виде матриц $4$x$4$ и проведите расчеты для получения $c_{1a}$ и $c_{1b}$ в виде векторов-столбцов.
# 
